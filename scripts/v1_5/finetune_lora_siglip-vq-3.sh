...
deepspeed --master_port 29515 --include=localhost:5,6,7 /public_data/jihai/understanding/llava/train/train_mem.py \
    ...
    --image_folder /public_data/jihai/data/multimodalout/smart_watch_image_train_120ku_180km \
    --vision_tower google/siglip-base-patch16-224 \
    ...
    --bf16 True \
    --tf32 True \
    --output_dir ./checkpoints/llava-v1.5-7b-siglip-vq_120ku_180km-sw-lora \
    --num_ckpt_to_save 17 \
    --num_train_epochs 1 \
...
python eval_generate_smartwatch.py \
  ...
  --ckpt_step 45 \
  --ckpt_num 3 \
  --model_name "llava-v1.5-7b-siglip-vq_120ku_180km-sw-lora" > output_gpu5.log 2>&1 &
...
python eval_generate_smartwatch.py \
  ...
  --ckpt_step 45 \
  --ckpt_num 3 \
  --model_name "llava-v1.5-7b-siglip-vq_120ku_180km-sw-lora" > output_gpu6.log 2>&1 &
...
python eval_generate_smartwatch.py \
  ...
  --ckpt_step 45 \
  --ckpt_num 4 \
  --model_name "llava-v1.5-7b-siglip-vq_120ku_180km-sw-lora" \
