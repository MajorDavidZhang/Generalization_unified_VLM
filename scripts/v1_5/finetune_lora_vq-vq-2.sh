...
deepspeed --master_port 29516 --include=localhost:5,6,7 /public_data/jihai/understanding/llava/train/train_mem.py \
    ...
    --vision_tower vq \
    --vision_tower_permutation_path /public_data/jihai/understanding/llava/model/multimodal_encoder/vqaffine_0.2.pth \
    --vision_tower_gen vq \
    ...
    --bf16 True \
    --tf32 True \
    --output_dir ./checkpoints/llava-v1.5-7b-vq-vq-af2-2-u-sw-lora \
    --num_ckpt_to_save 10 \
    --num_train_epochs 1 \
...
python eval_generate_smartwatch.py \
  ...
  --ckpt_step 30 \
  --ckpt_num 1 \
  --model_name "llava-v1.5-7b-vq-vq-af2-2-u-sw-lora" \
  --understanding_only
